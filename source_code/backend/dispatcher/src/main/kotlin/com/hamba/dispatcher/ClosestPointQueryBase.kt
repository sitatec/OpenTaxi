...
import dilivia.s2.region.*
import mu.KotlinLogging
import org.apache.commons.math3.util.FastMath
import java.util.*

/**
 * A custom version of S2ClosestPointQueryBase
 */
class ClosestPointQueryBase<T: Distance<T>, D : Comparable<D>> {
    ...
    /** The point index. */
    private lateinit var index: PointIndex<D>
    /**
     * Get a reference to the underlying PointIndex.
     * @return The index of the query.
     */
    fun index(): PointIndex<D> = index
    /** The query options. */
    ...
    private lateinit var iter: PointIndex.Iterator<D>
     ...
     */
    constructor(distanceFactory: DistanceFactory<T>, index: PointIndex<D>): this(distanceFactory)  {
        init(index)
    }
     ...
     * @param index The index on which the query is executed.
     */
    fun init(index: PointIndex<D>) {
        this.index = index
        this.iter = PointIndex.Iterator(index)
        reInit()
    }
    ...
    fun findClosestPoints(target: S2DistanceTarget<T>, options: Options<T>, results: MutableList<Result<T, D>>) {
        ...
        if (options.getMaxResult() == 1) {
            ...
            results.reverse()
            PreConditions.checkState { results.isSorted() }
        }
    ...
    fun findClosestPoint(target: S2DistanceTarget<T>, options: Options<T>): Result<T, D> {
        PreConditions.requireEQ(options.getMaxResult(), 1)
        findClosestPointsInternal(target, options)
    ...
    private fun findClosestPointsInternal(target: S2DistanceTarget<T>, options: Options<T>) {
        ...
        PreConditions.requireArgument { resultVector.isEmpty() }
        PreConditions.requireArgument { resultSet.isEmpty() }
        PreConditions.requireGE(target.maxBruteForceIndexSize(), 0)
    ...
    private fun initQueue() {
        PreConditions.requireArgument { queue.isEmpty() }
    ...
    private fun addInitialRange(firstId: S2CellId, lastId: S2CellId) {
        ...
        PreConditions.requireGE(level, 0)
    ...
    private fun processOrEnqueue(id: S2CellId, iter: PointIndex.Iterator<D>, seek: Boolean): Boolean {
    ...
    open class Options<T : Distance<T>>(
        ...
        fun setMaxResult(value: Int) {
            PreConditions.requireGE(value, 1)
    ...
    private fun S2CellId.getCommonAncestorLevel(other: S2CellId): Int {
        ...
        PreConditions.checkState { bits != 0UL }  // Because lsb() is non-zero.
    ...
    private fun S2CellId.lsb(): ULong {
